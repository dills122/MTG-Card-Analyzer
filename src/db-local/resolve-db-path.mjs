...
// Shared by every local nedb store (names, hash cache, ops log, collection, needs-attention).
...
// preferredPath: explicit configured path (dir or .db file), tried first.
// fileName: default filename used when a candidate is a directory rather than a .db file.
function resolveDbFilename(preferredPath, fileName) {
    ...
    for (const candidate of candidates) {
        const dbFile = toDbFilePath(candidate, fileName);
        if (canUsePath(dbFile)) {
            return dbFile;
        }
    }

    return path.join(os.tmpdir(), "mtg-card-analyzer", fileName);
}

export { resolveDbFilename, toDbFilePath, canUsePath };
